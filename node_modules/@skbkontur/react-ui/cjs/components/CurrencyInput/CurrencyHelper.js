"use strict";var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");exports.__esModule = true;exports.CurrencyHelper = void 0;var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/extends"));
...
CurrencyHelper = /*#__PURE__*/function () {function CurrencyHelper() {}CurrencyHelper.
  ...
  getOptions = function getOptions(options) {
    return (0, _extends2.default)({}, CurrencyHelper.defaultOptions, {}, options);
  };CurrencyHelper.

  getInfo = function getInfo(value, options) {
  ...
  getCursorMap = function getCursorMap(formatted, formattingOptions) {
  ...
  unformatString = function unformatString(value) {
    return value.
    replace(/\s/g, '').
    replace(',', '.').
    replace(/[\u2212\u002D\uFE63\uFF0D\u2012\u2013\u2014\u2015]/g, '-');
  ...
  formatForClipboard = function formatForClipboard(value) {
    return CurrencyHelper.unformatString(value).replace('.', ',');
  };CurrencyHelper.

  formatString = function formatString(value, formattingOptions) {
    var options = CurrencyHelper.getOptions(formattingOptions);
    value = CurrencyHelper.unformatString(value);
    var destructed = CurrencyHelper.destructString(value) || { sign: '', integer: '', delimiter: '', fraction: '' };var

    sign = destructed.sign,integer = destructed.integer,delimiter = destructed.delimiter;
    ...
    if (delimiter || fractionDigits) {
      result += ',';
      result += fraction.padEnd(Math.min(fractionDigits, _constants.MAX_SAFE_DIGITS), '0');
    }
  ...
  isValidString = function isValidString(value, options) {
    value = CurrencyHelper.unformatString(value);
    ...
    sign = destructed.sign,integer = destructed.integer,delimiter = destructed.delimiter,fraction = destructed.fraction;
    ...
    if (options.integerDigits == null && integerDigits > _constants.MAX_SAFE_DIGITS - (options.fractionDigits || 0)) {
      return false;
    }
    ...
    return integerDigits + fractionDigits <= _constants.MAX_SAFE_DIGITS;
  };CurrencyHelper.

  extractValid = function extractValid(value, options) {
    value = CurrencyHelper.unformatString(value);
    ...
    var token = match[0].substr(0, _constants.MAX_ALLOWED_CHARS);
  ...
  destructString = function destructString(value) {
    ...
    match[1],sign = _match$ === void 0 ? '' : _match$,_match$2 = match[2],integer = _match$2 === void 0 ? '' : _match$2,_match$3 = match[3],delimiter = _match$3 === void 0 ? '' : _match$3,_match$4 = match[4],fraction = _match$4 === void 0 ? '' : _match$4;
    return { sign: sign, integer: integer, delimiter: delimiter, fraction: fraction };
  };return CurrencyHelper;}();exports.CurrencyHelper = CurrencyHelper;CurrencyHelper.defaultOptions = { fractionDigits: null, hideTrailingZeros: false, thousandsDelimiter: String.fromCharCode(0x2009), minusSign: String.fromCharCode(0x2212) };
"use strict";exports.__esModule = true;exports.MouseDrag = void 0;var MouseDragEventType;(function (MouseDragEventType) {MouseDragEventType["Start"] = "mousedragstart";MouseDragEventType["Move"] = "mousedragmove";MouseDragEventType["End"] = "mousedragend";MouseDragEventType["Leave"] = "mousedragleave";})(MouseDragEventType || (MouseDragEventType = {}));
...
var documentHandleMouseUp = function documentHandleMouseUp(e) {return items.forEach(function (mouseDrag) {return mouseDrag.handleMouseUp(e);});};
...
function MouseDrag(elem) {var _this = this;this.mouseDragStartEvent = void 0;this.clicked = false;this.dragging = false;this.x1 = void 0;this.y1 = void 0;this.elem = void 0;this.
  ...
  onMouseDragStart = function (handler) {return _this.on(MouseDragEventType.Start, handler);};this.
  onMouseDragMove = function (handler) {return _this.on(MouseDragEventType.Move, handler);};this.
  onMouseDragLeave = function (handler) {return _this.on(MouseDragEventType.Leave, handler);};this.
  onMouseDragEnd = function (handler) {return _this.on(MouseDragEventType.End, handler);};this.
  ...
  dispatchEvent = function (mouseDragEvent) {
    if (_this.elem !== null) {
      _this.elem.dispatchEvent(mouseDragEvent);
    }
  };this.elem = elem;this.elem.addEventListener('mousedown', this.handleMouseDown);this.elem.addEventListener('mousemove', this.handleMouseMove);this.elem.addEventListener('mouseleave', this.handleMouseLeave);items.set(this.elem, this);};exports.MouseDrag = MouseDrag;MouseDrag.RADIUS = 5;MouseDrag.listen = function (elem) {if (items.has(elem)) {var mouseDrag = items.get(elem);if (mouseDrag) {return mouseDrag;}}return new MouseDrag(elem);};MouseDrag.stop = function (elem) {if (elem && items.has(elem)) {var mouseDrag = items.get(elem);if (mouseDrag) {mouseDrag.stop();}}};
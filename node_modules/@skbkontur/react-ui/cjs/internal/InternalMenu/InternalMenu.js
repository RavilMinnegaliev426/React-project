...
InternalMenu = /*#__PURE__*/function (_React$Component) {(0, _inheritsLoose2.default)(InternalMenu, _React$Component);function InternalMenu() {var _this;for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {args[_key] = arguments[_key];}_this = _React$Component.call.apply(_React$Component, [this].concat(args)) || this;_this.
    ...
    renderHeader = function () {var _cn;
      return (
        _react.default.createElement("div", {
          ref: function ref(el) {return _this.header = el;},
          className: (0, _classnames.default)((_cn = {}, _cn[
          _InternalMenu.jsStyles.header()] = true, _cn[
          _InternalMenu.jsStyles.fixedHeader()] = _this.state.scrollState !== 'top', _cn)) },
    ...
    renderFooter = function () {var _cn2;
      return (
        _react.default.createElement("div", {
          ref: function ref(el) {return _this.footer = el;},
          className: (0, _classnames.default)((_cn2 = {}, _cn2[
          _InternalMenu.jsStyles.footer()] = true, _cn2[
          _InternalMenu.jsStyles.fixedFooter()] = _this.state.scrollState !== 'bottom', _cn2)) },
    ...
    shouldRecalculateMaxHeight = function (prevProps) {var _this$props =
      _this.props,maxHeight = _this$props.maxHeight,header = _this$props.header,footer = _this$props.footer,children = _this$props.children;
      var prevMaxHeight = prevProps.maxHeight;
      var prevHeader = prevProps.header;
      var prevFooter = prevProps.footer;
      var prevChildrenCount = _react.default.Children.count(prevProps.children);

      return (
        maxHeight !== prevMaxHeight ||
        footer !== prevFooter ||
        header !== prevHeader ||
        _react.default.Children.count(children) !== prevChildrenCount);
    ...
    handleKeyDown = function (e) {
      ...
      if ((0, _identifiers.isKeyArrowUp)(e)) {
        e.preventDefault();
        _this.moveUp();
      } else if ((0, _identifiers.isKeyArrowDown)(e)) {
        e.preventDefault();
        _this.moveDown();
      } else if ((0, _identifiers.isKeyEnter)(e)) {
    ...
    handleScrollStateChange = function (scrollState) {
      if (_this.state.scrollState !== scrollState) {
        _this.setState({ scrollState: scrollState });
      }
    };return _this;}var _proto = InternalMenu.prototype;_proto.componentDidMount = function componentDidMount() {this.setInitialSelection();this.calculateMaxHeight();};_proto.componentDidUpdate = function componentDidUpdate(prevProps, prevState) {if (this.shouldRecalculateMaxHeight(prevProps)) {this.calculateMaxHeight();}};_proto.UNSAFE_componentWillReceiveProps = function UNSAFE_componentWillReceiveProps(nextProps) {if (nextProps.maxHeight !== this.props.maxHeight) {this.setState({ maxHeight: nextProps.maxHeight || 'none' });}};_proto.focus = function focus() {this.focusOnRootElement();};_proto.render = function render() {var _this2 = this;return _react.default.createElement(_ThemeContext.ThemeContext.Consumer, null, function (theme) {_this2.theme = theme;return _this2.renderMain();});};_proto.renderMain = function renderMain() {var _cn3,_this3 = this;var enableIconPadding = _react.default.Children.toArray(this.props.children).some(function (x) {return _react.default.isValidElement(x) && x.props.icon;});if (this.isEmpty()) {return null;}return _react.default.createElement("div", { className: (0, _classnames.default)((_cn3 = {}, _cn3[_InternalMenu.jsStyles.root(this.theme)] = true, _cn3[_InternalMenu.jsStyles.shadow()] = this.props.hasShadow, _cn3)), style: { width: this.props.width, maxHeight: this.state.maxHeight }, onKeyDown: this.handleKeyDown, ref: function ref(element) {_this3.rootElement = element;}, tabIndex: 0 }, this.props.header ? this.renderHeader() : null, _react.default.createElement(_ScrollContainer.ScrollContainer, { ref: this.refScrollContainer, maxHeight: this.props.maxHeight, preventWindowScroll: this.props.preventWindowScroll, onScrollStateChange: this.handleScrollStateChange }, _react.default.Children.map(this.props.children, function (child, index) {if (typeof child === 'string' || typeof child === 'number' || child == null) {return child;}if (_react.default.isValidElement(child) && typeof child.type === 'string') {return child;}if (enableIconPadding && ((0, _MenuItem.isMenuItem)(child) || (0, _MenuHeader.isMenuHeader)(child))) {child = _react.default.cloneElement(child, { _enableIconPadding: true });}if ((0, _isActiveElement.isActiveElement)(child)) {var highlight = _this3.state.highlightedIndex === index;var ref = child.ref;var originalRef = ref;if (highlight) {ref = function ref(menuItem) {return _this3.refHighlighted(originalRef, menuItem);};}return _react.default.cloneElement(child, { ref: ref, state: highlight ? 'hover' : child.props.state, onClick: _this3.select.bind(_this3, index, false), onMouseEnter: function onMouseEnter(event) {_this3.highlightItem(index);if ((0, _MenuItem.isMenuItem)(child) && child.props.onMouseEnter) {child.props.onMouseEnter(event);}}, onMouseLeave: function onMouseLeave(event) {_this3.unhighlight();if ((0, _MenuItem.isMenuItem)(child) && child.props.onMouseLeave) {child.props.onMouseLeave(event);}} });}return child;})), this.props.footer ? this.renderFooter() : null);};_proto.refHighlighted = function refHighlighted(originalRef, menuItem) {this.highlighted = menuItem;if (!originalRef || typeof originalRef === 'string') {return;}if (typeof originalRef === 'function') {originalRef(menuItem);} else if (typeof originalRef === 'object') {// @ts-ignore see issue https://github.com/DefinitelyTyped/DefinitelyTyped/issues/31065
